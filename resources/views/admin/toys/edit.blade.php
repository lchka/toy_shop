            ...
            <div class="my-6 p-6 bg-white border-b border-gray-200 shadow-sm sm:rounded-lg">
                ...
                <form action="{{ route('admin.toys.update', $toy) }}" method="post" enctype="multipart/form-data">
                    ...
                    <x-text-input type="text" name="name" field="name" placeholder="Name" class="w-full"
                        autocomplete="off" :value="@old('name', $toy->name)"></x-text-input>
                    @error('name')
                    ...
                    <x-text-input type="text" name="type" field="type" placeholder="Type" class="w-full mt-6"
                        autocomplete="off" :value="@old('type', $toy->type)"></x-text-input>
                    @error('type')
                    <span class="text-red-500">{{ $message }}</span>
                    @enderror
                    ...
                    <select class="custom-select w-full mt-6" type="select" name="colour" id="colour" field="colour">
                        @foreach(['red', 'green',
                        'mixed','black','white','orange','purple','blue','yellow','pink','brown'] as $colour)
                        <option value="{{ $colour }}" {{ old('colour')===$colour ? 'selected' : '' }}>{{
                            ucfirst($colour) }}</option>
                        @endforeach
                    </select>
                    ...
                    <select class="custom-select" type="select" name="size" id="size" field="size" class="w-full mt-6">
                    ...
                    <x-textarea type="text" name="description" rows="10" field="description"
                        placeholder="Description..." class="w-full mt-6"
